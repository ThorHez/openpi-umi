...
class Pi0Hybrid(_model.BaseModel):
    ...
    def compute_hybrid_loss(
        ...
        *,
        ...
        # 1. Compute Flow Matching loss (primary)
        flow_loss_per_step = self.compute_loss(flow_rng, observation, actions, train=train)
        flow_loss = jnp.mean(flow_loss_per_step)
        
        # 2. Compute FAST loss (auxiliary) - if enabled
        if self.use_fast_loss:
            # NOTE: Full FAST loss implementation requires:
            # - Pre-tokenized actions in the data pipeline
            # - Autoregressive prediction head
            # For now, we provide a simplified version
            
            # Placeholder: FAST loss would compute CE on tokenized action prediction
            # This requires the data pipeline to provide tokenized actions
            fast_loss = jnp.zeros(())  # Placeholder
            
            # Full implementation would look like:
            # fast_loss = self._compute_fast_loss(fast_rng, observation, actions, train=train)
        else:
            fast_loss = jnp.zeros(())
