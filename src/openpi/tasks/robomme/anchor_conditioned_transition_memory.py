"""Anchor-conditioned recurrent semantic table with teacher-forced feedback.

The module keeps the soft-gated write/swap updater used by the decomposed
RoboMME MEM, but makes each local operation explicitly conditional on the
previous semantic table.  Training can therefore move from privileged previous
states to straight-through hard student feedback without changing inference.
"""
...
def _straight_through_table(table: jnp.ndarray) -> jnp.ndarray:
    table = table / jnp.maximum(table.sum(axis=-1, keepdims=True), 1e-6)
    hard = jax.nn.one_hot(jnp.argmax(table, axis=-1), table.shape[-1])
    return table + jax.lax.stop_gradient(hard - table)


class SemanticOperationScanCell(nn.Module):
    """Predict and apply two shared semantic micro-operations for one chunk."""

    width: int = 64
    hidden_width: int = 128
    micro_events: int = 2
    gate_temperature: float = 0.25
    straight_through_hard_feedback: bool = True
    hard_event_commit: bool = False
    event_head_mode: str = "joint"
    ...
    def __call__(
        ...
        if self.use_auxiliary_heads:
            ...
            phase_hidden = nn.LayerNorm(name="phase_hidden_ln")(
                nn.Dense(self.width, name="phase_hidden")(phase_input)
            )
            phase_logits = nn.Dense(4, name="phase_logits")(nn.gelu(phase_hidden))
        else:
            completion_logits = jnp.zeros(
                (batch, self.micro_events), dtype=event_type_logits.dtype
            )
            event_kind_logits = jnp.zeros(
                (batch, self.micro_events, 2), dtype=event_type_logits.dtype
            )
            phase_logits = jnp.zeros((batch, 4), dtype=event_type_logits.dtype)
        ...
        for region_a, region_b in SWAP_PAIRS:
            ...
            pair_features.append(jnp.concatenate((a + b, jnp.abs(a - b), a * b), axis=-1))
        ...
        def event_gates(type_logits, completion_logit, kind_logits):
            if self.event_head_mode == "joint":
                joint = jax.nn.softmax(
                    type_logits.astype(jnp.float32) / self.gate_temperature,
                    axis=-1,
                )
                completion = 1.0 - joint[:, 0]
                kind = joint[:, 1:] / jnp.maximum(
                    joint[:, 1:].sum(axis=-1, keepdims=True), 1e-6
                )
            elif self.event_head_mode == "completion":
                # Completion BCE is trained on the raw logit. Keep inference on
                # that same probability scale so calibrated commit thresholds
                # retain their meaning; gate_temperature is only for the
                # categorical operation kind.
                completion = jax.nn.sigmoid(completion_logit.astype(jnp.float32))
                kind = jax.nn.softmax(
                    kind_logits.astype(jnp.float32) / self.gate_temperature,
                    axis=-1,
                )
            else:
                raise ValueError(f"Unknown event head mode: {self.event_head_mode}")
            soft = jnp.concatenate(
                ((1.0 - completion)[:, None], completion[:, None] * kind), axis=-1
            )
            if not self.hard_event_commit:
                return soft
            hard_completion = completion >= self.commit_threshold
            hard_kind = jax.nn.one_hot(jnp.argmax(kind, axis=-1), 2)
            hard = jnp.concatenate(
                (
                    (~hard_completion)[:, None],
                    hard_completion[:, None] * hard_kind,
                ),
                axis=-1,
            ).astype(jnp.float32)
            return soft + jax.lax.stop_gradient(hard - soft)
        ...
        def apply_micro(
            ...
            gates = event_gates(type_logits, completion_logit, kind_logits)
        ...
        candidate = feedback
        committed_event_gates = []
        for micro in range(self.micro_events):
            gates = event_gates(
                event_type_logits[:, micro],
                completion_logits[:, micro],
                event_kind_logits[:, micro],
            )
            committed_event_gates.append(gates)
        ...
        return next_table, (
            ...
            event_kind_logits,
            phase_logits,
            jnp.stack(committed_event_gates, axis=1),
        )


class AnchorConditionedTransitionMemory(nn.Module):
    """Shared anchor/pair evidence plus recurrent semantic feedback."""
    ...
    def __call__(
        ...
        *,
        ...
        del train
        if self.event_head_mode == "completion" and not self.use_auxiliary_heads:
            raise ValueError("completion event head requires auxiliary heads")
        if not 0.0 < self.commit_threshold < 1.0:
            raise ValueError("commit threshold must be inside (0, 1)")
        ...
        if teacher_previous_tables.shape != (batch, self.max_steps, 7):
            raise ValueError(f"Invalid teacher table shape {teacher_previous_tables.shape}")
        ...
        coordinates = jnp.stack(
            (
                y, x, y * y, x * x, y * x,
                jnp.sin(jnp.pi * y), jnp.cos(jnp.pi * y),
        ...
        result = {
            ...
            "committed_event_gates": event_gates,
            "write_entity_logits": entity_logits,
