"""Localize one-swap information loss before the final memory compressor.
...
Pipeline:
    30 x 256 frozen SigLIP patch tokens
      ...
      -> the same learned-query classifier used by the successful raw-patch probe.

Unlike ``train_one_swap_framewise_temporal_memory_probe.py``, this diagnostic
does not create the final M-token Pi0-width memory.  It therefore distinguishes
loss caused by early framewise compression from loss caused by final memory
compression.
"""
...
class FramewiseTemporalPreMemoryCheckpointLoader:
    """Restore the policy while randomly initializing the diagnostic path."""
    ...
    def load(self, params: at.Params) -> at.Params:
        ...
        return weight_loaders._merge_params(
            ...
            missing_regex=(
                ...
                r"HistoryFramewiseTemporalPreMemory).*"
            ),
        )


class FramewiseTemporalPreMemoryProbe(nn.Module):
    """Classify directly from the contextualized K slots of all frames."""
    ...
    def __call__(self, patch_tokens, *, train: bool):
        ...
        if (
            patch_tokens.shape[1] != self.num_frames
            or patch_tokens.shape[-1] != self.input_width
        ):
            raise ValueError(
                f"Expected T={self.num_frames}, D={self.input_width}; "
        ...
        flat = slots.reshape(batch_size, -1, self.width)
        ...
        query = jnp.tile(readout_query, (batch_size, 1, 1))
...
@dataclasses.dataclass(frozen=True)
class OneSwapFramewiseTemporalPreMemoryConfig(_base_model.Pi0MemCompressConfig):
    ...
    ) -> OneSwapFramewiseTemporalPreMemoryModel:
        return OneSwapFramewiseTemporalPreMemoryModel(self, rngs=nnx.Rngs(rng))

    def get_freeze_filter_pre_memory(self) -> nnx.filterlib.Filter:
        probe = nnx_utils.PathRegex(r".*HistoryFramewiseTemporalPreMemory.*")
        return nnx.Not(probe)


class OneSwapFramewiseTemporalPreMemoryModel(_base_model.Pi0MemCompress):
    def __init__(
        self,
        config: OneSwapFramewiseTemporalPreMemoryConfig,
        rngs: nnx.Rngs,
    ):
        super().__init__(config, rngs)
        self.HistoryFramewiseTemporalPreMemory = nnx_bridge.ToNNX(
            FramewiseTemporalPreMemoryProbe(
                num_frames=30,
                input_width=1152,
        ...
        self.HistoryFramewiseTemporalPreMemory.lazy_init(
            fake_tokens,
            train=False,
            rngs=rngs,
        )
    ...
    def compute_history_classification(
        ...
        *,
        ...
        if image.ndim != 5 or image.shape[1] != 31:
            raise ValueError(
                f"Pre-memory probe expects [B,31,H,W,C], got {image.shape}"
            )

        _, encoder_out = self.PaliGemma.img(image, train=False)
        history_patches = encoder_out["with_posemb"][:, :30]
        logits, pooled = self.HistoryFramewiseTemporalPreMemory(
            history_patches,
            train=train,
        )
        return logits, {
...
def build_config(
    ...
    model = OneSwapFramewiseTemporalPreMemoryConfig(
        pi05=True,
        action_dim=32,
        action_horizon=16,
    ...
    return _config.TrainConfig(
        name="pi0_shellgame_one_swap_framewise_temporal_pre_memory_probe_260807",
        exp_name=args.exp_name,
        model=model,
        freeze_filter=model.get_freeze_filter_pre_memory(),
        ...
        weight_loader=FramewiseTemporalPreMemoryCheckpointLoader(SOURCE_CHECKPOINT),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--exp-name",
        default="one_swap_framewise_temporal_pre_memory",
    )
