#!/usr/bin/env python3
"""Serve frozen V6 plus the bounded current-visual XY correction head."""
...
from openpi.training.mem.recipes import shellgame_qwen_distilled_memory_waypoint_visual_correction_v6 as recipe
...
def _load_policy(
    ...
    model_config = dataclasses.replace(
        recipe.make_model_config(), correction_scale=correction_scale
    )
...
def main() -> None:
    ...
    policy = DeterministicNoisePolicy(
        _load_policy(checkpoint, args.num_sampling_steps, args.correction_scale)
    )
    logging.info(
        "Loaded visual-correction waypoint policy checkpoint=%s correction_scale=%.3f",
        checkpoint,
