...
JAX_COMPILATION_CACHE_DIR=/data2/hzl_workspace_for_pi_mem/.codex_tmp/jax_cache_eef_v7_eval_${LABEL}
SERVER_LOG=${OPENPI_ROOT}/serve_eef_v7_${LABEL}_260818.log
...
CUDA_VISIBLE_DEVICES="${GPU}" XLA_PYTHON_CLIENT_MEM_FRACTION=0.85 \
  setsid uv run python examples/shellgame/serve_old_tracker_full_absolute_eef.py \
    --checkpoint-dir "${CHECKPOINT}" \
    --port "${PORT}" \
    --num-sampling-steps 4 >"${SERVER_LOG}" 2>&1 &
server_pid=$!

# A newly written OCDBT checkpoint can take more than four minutes to restore
# from a cold, nearly-full data volume.  Keep the evaluation contract unchanged
# while allowing up to twenty minutes for model restore and JAX initialization.
ready=0
