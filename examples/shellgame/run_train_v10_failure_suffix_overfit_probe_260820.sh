...
STEPS=${STEPS:-300}
EXP_NAME=${EXP_NAME:-absolute_eef7_v10_failure_suffix_exact3_xy4_b6_${STEPS}steps_6gpu_260820}
...
OUTPUT=${OPENPI_ROOT}/checkpoints/pi0_shellgame_v10_failure_suffix_overfit_probe_260820/${EXP_NAME}
LOG=${OPENPI_ROOT}/train_eef7_v10_failure_suffix_overfit_probe_${STEPS}steps_260820.log
...
exec >>"${LOG}" 2>&1
echo "[$(date --iso-8601=seconds)] Starting exact-state V10 failure suffix overfit probe"
echo "physical_gpus=${GPU_IDS} fsdp_devices=6 steps=${STEPS}"
echo "dataset=exact_ep0_ep1_ep17_oracle_suffix xy_loss_weight=4 gripper_loss_weight=4"
echo "init=${INIT} output=${OUTPUT}"

CUDA_VISIBLE_DEVICES=${GPU_IDS} XLA_PYTHON_CLIENT_MEM_FRACTION=0.90 \
uv run python examples/shellgame/train_v10_failure_suffix_overfit_probe.py \
  --exp-name "${EXP_NAME}" \
