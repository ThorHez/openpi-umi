"""Run fixed-history ShellGame evaluation with one fresh process per episode.

Long-lived EGL + WebSocket evaluation processes eventually return corrupted
camera readbacks on this machine. Keeping the policy server alive while
running each Robosuite episode in a spawned child process releases the EGL
context at every episode boundary. Periodic WebSocket reconnection and a
bounded physics trace prevent large history/debug buffers from accumulating
within one episode.
"""
...
class Args(joint.Args):
    num_trials: int = 20
    seed: int = 0
    video_out_path: str = "evaluation/shellgame/fixed_history_isolated"
...
def _run_episode(
    ...
    *,
    ...
    class ReconnectingWebsocketClient:
        """Bound the lifetime of large fixed-history WebSocket connections."""
    ...
    logging.info(
        "isolated episode=%d pid=%d seed=%d initial=%s swaps=%d",
        episode_index,
...
def main(args: Args) -> None:
    ...
    for spec in specs:
        ...
        logging.info(
            "isolated aggregate: episodes=%d/%d lift=%d/%d selection=%d/%d",
            len(results),
            args.num_trials,
            successes,
            len(results),
    ...
    summary = {
        "evaluation": "one fresh MuJoCo EGL and WebSocket client process per episode",
