...
LOG=${OPENPI_ROOT}/train_eef7_mixed_correction_v8_optimized_dynamic_2k_260818.log
AUDIT=${DATA}/sustained_recovery_v8_optimized_oracle_supervision_audit.json

cd "${OPENPI_ROOT}"
[[ -d "${DATA}" ]] || { echo "Missing converted optimized-V8 dataset: ${DATA}" >&2; exit 1; }
[[ -f "${AUDIT}" ]] || { echo "Missing optimized-V8 conversion audit: ${AUDIT}" >&2; exit 1; }
[[ -d "${INIT}" ]] || { echo "Missing V6 initialization: ${INIT}" >&2; exit 1; }
[[ ! -e "${OUTPUT}" ]] || { echo "Refusing to overwrite ${OUTPUT}" >&2; exit 1; }

# Leave room for two retained ~9 GiB checkpoints, one asynchronous checkpoint
# being committed, local dataset/JAX caches, and unrelated concurrent writes.
available_kb=$(df -Pk "${WORKSPACE}" | awk 'NR==2 {print $4}')
required_kb=$((100 * 1024 * 1024))
echo "disk_free_gib=$((available_kb / 1024 / 1024)) required_gib=100"
if (( available_kb < required_kb )); then
  echo "Refusing to train with less than 100 GiB free" >&2
  exit 1
fi
...
exec >>"${LOG}" 2>&1
echo "[$(date --iso-8601=seconds)] Starting optimized-V8 dynamic-count training"
echo "physical_gpus=${GPU_IDS} fsdp_devices=6"
CUDA_VISIBLE_DEVICES=${GPU_IDS} XLA_PYTHON_CLIENT_MEM_FRACTION=0.90 \
uv run python examples/shellgame/train_old_tracker_full_absolute_eef_mixed_correction_v8_optimized.py \
  --exp-name "${EXP_NAME}" \
