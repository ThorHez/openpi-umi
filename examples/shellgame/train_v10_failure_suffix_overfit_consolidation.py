"""Low-LR single-expert consolidation probe for failed V10 suffixes.

This probe starts from the best early uniform-overfit checkpoint (step 100),
which already retains descent/grasp/lift.  It then rebalances the exact same
six recorded episodes to 30% recenter, 30% descend, 20% grasp, and 20% lift.
Memory and the tracker stay frozen; only the action expert is updated.
"""
...
CONFIG_NAME = "pi0_shellgame_v10_failure_suffix_overfit_consolidation_260820"
ROWS_PER_EPISODE = {
    "recenter_60_69": 30,
    "descend_70_99": 30,
    "grasp_100_114": 20,
...
def _consolidation_indices(dataset, indices: list[int], classifier_config) -> list[int]:
    ...
    if episodes.size == 0:
        raise RuntimeError("Consolidation probe found no Oracle-aligned rows")

    masks = {
        "recenter_60_69": (eligible_frame >= 60) & (eligible_frame <= 69),
        "descend_70_99": (eligible_frame >= 70) & (eligible_frame <= 99),
        "grasp_100_114": (eligible_frame >= 100) & (eligible_frame <= 114),
        "lift_115_154": (eligible_frame >= 115) & (eligible_frame <= 154),
    }
    ...
    for group_index, (name, mask) in enumerate(masks.items()):
        ...
        resized.append(
            _resize_per_episode(
                ...
                seed=260840 + group_index * 1_000,
            )
        )

    merged = np.concatenate(resized)
    merged = np.random.default_rng(260844 + len(indices)).permutation(merged)
    ...
    logging.info(
        "Exact-state consolidation sampler: episodes=%s input=%d raw=%s "
        "targets_per_episode=%s output=%d",
        episodes.tolist(),
        len(indices),
        raw_counts,
        ROWS_PER_EPISODE,
...
def main() -> None:
    ...
    if not 100 <= args.steps <= 200:
        raise ValueError("Keep the consolidation probe between 100 and 200 steps")
    _base._validate_data()  # noqa: SLF001
    logging.info(
        "Single-expert consolidation: recenter=30%% descend=30%% grasp=20%% "
        "lift=20%% init=%s tracker+memory=frozen",
        args.init_checkpoint,
    )
    _config_pi0_mem.VideoFrameDataset = _full_joint.FixedPrefixCurrentVideoDataset
    _trainer._filter_memory_classifier_frame_range = _consolidation_indices
