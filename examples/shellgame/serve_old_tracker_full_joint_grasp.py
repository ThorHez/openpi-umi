"""Serve the frozen old tracker with the full joint + gripper action expert."""
...
DEFAULT_CHECKPOINT = Path(
    ...
    "pi0_shellgame_old_tracker_full_joint_grasp_260810/"
    "full_joint_grasp_phase_balanced_b12_260810/1999"
)


def _build_config(*, sampling_steps: int):
    args = _original_args(episodes=20, batch_size=6, sampling_steps=sampling_steps)
    # Extra fields consumed only by the full-action training config builder.
    args.exp_name = "serve_old_tracker_full_joint_grasp"
    args.init_checkpoint = full_action.OLD_QUERY_ACTION_CHECKPOINT
...
def main() -> None:
    ...
    if not isinstance(policy._model, full_action.OldTrackerFullJointGraspModel):  # noqa: SLF001
        raise TypeError(f"Unexpected restored model: {type(policy._model).__name__}")  # noqa: SLF001
    logging.info(
        "Loaded %s from %s (fixed_history=60 total_frames=61 prompt=%r)",
