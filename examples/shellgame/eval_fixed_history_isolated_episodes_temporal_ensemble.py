"""Isolated-episode ShellGame evaluation with temporal action ensembling."""
...
class Args(temporal.Args):
    num_trials: int = 20
    seed: int = 0
    video_out_path: str = "evaluation/shellgame/fixed_history_isolated_temporal_ensemble"
...
def _run_episode(
    ...
    *,
    ...
    base._policy_env_action = temporal._temporal_ensemble_policy_env_action  # noqa: SLF001
    ...
    logging.info(
        "isolated temporal ensemble episode=%d pid=%d seed=%d initial=%s swaps=%d decay=%.3f",
        episode_index,
    ...
    debug_path = episode_dir / "temporal_ensemble_debug.json"
    debug_path.write_text(
        json.dumps(temporal.temporal_ensemble_diagnostics(), indent=2),
        encoding="utf-8",
    )
...
def _read_episode_result(root: pathlib.Path, spec: dict, *, save_videos: bool) -> dict:
    ...
    ensemble_path = episode_dir / "temporal_ensemble_debug.json"
    if not trace_path.is_file() or not ensemble_path.is_file():
    ...
    return {
        **spec,
        ...
        "num_replans": len(ensemble_payload),
        "video": str(videos[0]) if videos else None,
        "physics_trace": str(trace_path),
        "temporal_ensemble_trace": str(ensemble_path),
    }
...
def main(args: Args) -> None:
    ...
    for spec in specs:
        ...
        logging.info(
            "isolated temporal ensemble aggregate: episodes=%d/%d lift=%d/%d selection=%d/%d",
            len(results),
            args.num_trials,
            successes,
            len(results),
    ...
    summary = {
        "evaluation": "isolated episodes with temporal action ensembling",
        "ensemble_decay": args.ensemble_decay,
        "ensemble_weight_order": "oldest_to_newest_exponential_decay",
    ...
    logging.info("isolated temporal ensemble result=%s", result_path)
