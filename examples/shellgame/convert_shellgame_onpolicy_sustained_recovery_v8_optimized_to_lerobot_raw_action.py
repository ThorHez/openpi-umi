"""Convert optimized V8 raw episodes and audit horizon=16 supervision."""
...
import audit_onpolicy_eef_sustained_recovery_dataset_v8_optimized as audit_optimized
...
def main() -> None:
    ...
    if not args.phase_instructions:
        raise ValueError("Optimized V8 conversion requires --phase-instructions")
    grasp_phases = {int(x.strip()) for x in args.grasp_phase_ids.split(",") if x.strip()}
    if grasp_phases != {8, 9, 10, 11}:
        raise ValueError("Optimized V8 conversion requires --grasp-phase-ids 8,9,10,11")

    input_paths = common.find_npz_paths(args.input, args.max_episodes)
    raw_audit = audit_optimized.audit(
        Path(args.input).expanduser().resolve(),
        expected_episodes=len(input_paths),
    )
    raw.main()
    ...
    payload = {
        "ok": True,
        "dataset_kind": f"{optimized.DATASET_KIND}_lerobot",
        "raw_audit": raw_audit,
    ...
    path = output / "sustained_recovery_v8_optimized_oracle_supervision_audit.json"
