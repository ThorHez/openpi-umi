"""Replace only the learned initial-slot embedding in the successful M128 probe.

The successful memory-token sweep projects the exact initial slot from 3 to 64
dimensions before adding it to memory token 0.  This controlled ablation uses
the original sparse interface instead: the one-hot is added directly to the
first three channels of token 0.  Pair tokens, M=128, width=64 float32 math,
the original depth-2 attention updater, readout, optimizer, and labels remain
unchanged.
"""
...
class RawInitialOracleMemoryTokenTracker(nn.Module):
    """M128 tracker with sparse three-channel initial-slot injection."""
    ...
    def __call__(self, initial_slots, swap_pair_ids):
        ...
        readout = _sweep.SharedMemoryTokenReadout(name="shared_readout", width=self.width)
        stage_logits = []
        stage_memories = []
...
class RawInitialOracleMemoryTokenConfig(_sweep.OracleMemoryTokenConfig):
    def create(self, rng: at.KeyArrayLike) -> RawInitialOracleMemoryTokenModel:
        return RawInitialOracleMemoryTokenModel(self, rngs=nnx.Rngs(rng))


class RawInitialOracleMemoryTokenModel(_oracle_pair.OraclePairProbeModel):
    def __init__(self, config: RawInitialOracleMemoryTokenConfig, rngs: nnx.Rngs):
        ...
        self.HistoryThreeSwapOraclePairRecurrentMemoryTracker = nnx_bridge.ToNNX(
            RawInitialOracleMemoryTokenTracker(
...
def run_self_test() -> None:
    tracker = RawInitialOracleMemoryTokenTracker(width=16, depth=2, num_heads=4, num_memory_tokens=8)
    ...
    if not all(causal) or not all(pair_effects) or not initial_effect:
        raise AssertionError(
            f"Raw-initial self-test failed: causal={causal}, pair={pair_effects}, initial={initial_effect}"
        )
    print(
        f"Raw-initial self-test passed: causal={causal}, pair_effects={pair_effects}, initial_effect={initial_effect}"
    )


def build_config(args: argparse.Namespace, labels_path: pathlib.Path) -> _config.TrainConfig:
    base_config = _sweep.build_config(args, labels_path)
    ...
    model = RawInitialOracleMemoryTokenConfig(**parent_fields)
    return dataclasses.replace(
        base_config,
        name="pi0_shellgame_three_swap_oracle_memory_token_raw_initial_260809",
        model=model,
