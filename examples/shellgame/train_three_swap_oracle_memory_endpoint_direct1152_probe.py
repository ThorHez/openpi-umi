"""Read projected M128 endpoints directly in 1152 dimensions.

This splits the failed endpoint chain in half.  The successful raw-initial,
width-64 M128 updater is still normalized and projected from 64 to 1152, but
the subsequent Dense(1152,64)+tanh in IntegratedCurrentReadout is removed.
Attention pooling and classification operate directly on 1152-D tokens.
"""
...
from examples.shellgame import train_three_swap_oracle_memory_endpoint_projection_probe as _endpoint
...
class Direct1152OracleMemoryTracker(nn.Module):
    """Project endpoints to 1152 and classify without a 64-D bottleneck."""
    ...
    def __call__(self, initial_slots, swap_pair_ids):
        ...
        logits = _sweep.SharedMemoryTokenReadout(
            name="shared_readout",
...
class Direct1152OracleMemoryConfig(_endpoint.EndpointProjectionOracleMemoryConfig):
    def create(self, rng: at.KeyArrayLike) -> Direct1152OracleMemoryModel:
        return Direct1152OracleMemoryModel(self, rngs=nnx.Rngs(rng))


class Direct1152OracleMemoryModel(_oracle_pair.OraclePairProbeModel):
    def __init__(self, config: Direct1152OracleMemoryConfig, rngs: nnx.Rngs):
...
def run_self_test() -> None:
    tracker = Direct1152OracleMemoryTracker(
        width=16,
        output_width=32,
        depth=2,
        num_heads=4,
    ...
    if not all(causal) or not all(pair_effects) or not initial_effect:
        raise AssertionError(
            f"Direct-1152 self-test failed: causal={causal}, pair={pair_effects}, initial={initial_effect}"
        )
    print(
        f"Direct-1152 self-test passed: causal={causal}, pair_effects={pair_effects}, initial_effect={initial_effect}"
    )
...
def build_config(args: argparse.Namespace, labels_path: pathlib.Path) -> _config.TrainConfig:
    ...
    model = Direct1152OracleMemoryConfig(
        **parent_fields,
        endpoint_output_width=args.endpoint_output_width,
    )
    return dataclasses.replace(
        base_config,
        name="pi0_shellgame_three_swap_oracle_memory_endpoint_direct1152_260809",
        model=model,
