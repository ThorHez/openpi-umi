...
def _maybe_binarize_gripper_features(
    batch: Dict[str, np.ndarray],
    dataset_config: Dict | None,
) -> Dict[str, np.ndarray]:
    """Apply optional per-feature binarization from YAML ``features.*_gripper_width.binarize``.

    By default: values with ``width > threshold`` get ``max_value``, else ``min_value``.
    If ``invert`` is true, the two output levels are swapped for the same threshold test.
    """
    ...
    for feat_name, feat_def in (dataset_config.get("features") or {}).items():
        ...
        arr = np.asarray(batch[feat_name], dtype=np.float32)
        hi_out, lo_out = (min_val, max_val) if invert else (max_val, min_val)
        batch[feat_name] = np.where(arr > threshold, hi_out, lo_out).astype(np.float32)
    return batch


def _print_gripper_binarize_config(dataset_config: Dict | None) -> None:
    """Print gripper ``binarize`` settings (effective defaults match ``_maybe_binarize_gripper_features``)."""
    print("\nGripper width binarize (YAML features.*_gripper_width.binarize):")
    if not dataset_config:
        print("  (no --config: binarize block not loaded from YAML)")
        return
    ...
    if not names:
        ...
        for feat_name in names:
            ...
            print(
                f"    when enabled: width > threshold -> {hi_out}, width <= threshold -> {lo_out}"
            )
...
def _write_one_episode_parquet(
    ...
    for key in hf_features:
        ...
        if ft.get("dtype") in ["image", "video"]:
        ...
        elif ft.get("dtype") in ("string",):
            # HF datasets expects a list[str] for string columns.
            # For single_frame_features this will typically be a scalar per frame.
            str_values: list[str] = []
            for frame_data in frames:
                ...
                if isinstance(raw, str):
                    str_values.append(raw)
                    continue
                arr = np.asarray(raw)
                if arr.size == 0:
                    str_values.append("")
                else:
                    str_values.append(str(arr.reshape(-1)[-1]))
            if str_values:
                # If some frames missed the key, pad to episode length.
                if len(str_values) != length:
                    str_values = (str_values + [""] * length)[:length]
                episode_buffer[key] = str_values
...
def _merge_value_target_params(
    dataset_config: Dict | None,
    *,
    c_fail_coef: float,
    value_clip_min: float,
    value_clip_max: float,
) -> Tuple[float, float, float]:
    """YAML ``value_target`` overrides; missing keys keep CLI defaults."""
    if not dataset_config:
        return c_fail_coef, value_clip_min, value_clip_max
    vt = dataset_config.get("value_target")
    if not isinstance(vt, dict):
        return c_fail_coef, value_clip_min, value_clip_max
    return (
        float(vt["c_fail_coef"]) if "c_fail_coef" in vt else c_fail_coef,
        float(vt["value_clip_min"]) if "value_clip_min" in vt else value_clip_min,
        float(vt["value_clip_max"]) if "value_clip_max" in vt else value_clip_max,
    )
...
def process_episode_fast_vectorized(
    ...
    # Binarize gripper widths on the full episode before sampling so observation horizons
    # (and action built from lowdim in SequenceSampler) see binarized values.
    _maybe_binarize_gripper_features(episode_data, dataset_config)
...
def _attach_value_targets_to_frames(
    flat: List[Tuple[int, List[Dict]]],
    *,
    c_fail_coef: float,
    clip_min: float,
    clip_max: float,
    ...
    def _frame_success_to_bool(frame: Dict[str, Any]) -> bool | None:
        if "success" not in frame:
            return None
        success_arr = np.asarray(frame["success"])
        if success_arr.size == 0:
            return None
        # success in LeRobot can be shaped like (H, 1); use the last timestep marker.
        return bool(success_arr.reshape(-1)[-1])

    episode_info: dict[int, value_targets.EpisodeTargetInfo] = {}
    task_max_lengths: dict[int, int] = {}

    for ep_idx, frames in flat:
        if not frames:
            continue
        task_name = str(frames[0].get("task", ""))
        task_index = int(task_to_index.get(task_name, 0))
        ...
        task_max_lengths[task_index] = max(task_max_lengths.get(task_index, 0), ep_length)

    for ep_idx, frames in flat:
        if not frames:
            continue
        frame_indices = np.arange(len(frames), dtype=np.int64)
        episode_indices = np.full((len(frames),), ep_idx, dtype=np.int64)
        ...
        for i, frame in enumerate(frames):
            frame["value_target"] = np.array([targets[i]], dtype=np.float32)
...
def convert_to_lerobot(
    ...
    c_fail_coef, value_clip_min, value_clip_max = _merge_value_target_params(
        dataset_config,
        c_fail_coef=c_fail_coef,
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Default failure penalty for value target; overridden by ``value_target.c_fail_coef`` in --config YAML when set.",
    )
    parser.add_argument(
        "--value-clip-min",
        type=float,
        default=-1.0,
        help="Default lower clip; overridden by ``value_target.value_clip_min`` in YAML when set.",
    )
    parser.add_argument(
        "--value-clip-max",
        type=float,
        default=0.0,
        help="Default upper clip; overridden by ``value_target.value_clip_max`` in YAML when set.",
    )
    ...
    convert_to_lerobot(
        ...
        value_clip_min=args.value_clip_min,
        value_clip_max=args.value_clip_max,
    )
