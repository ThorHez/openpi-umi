...
<<<<<<< HEAD
    def visualize_sample_frame_images(self, episode_idx: Optional[int] = None, 
        ...
        Args:
            episode_idx: 指定的episode索引，None则随机选择
            frame_idx: 指定的帧索引，None则随机选择
            output_path: 输出路径，None则直接显示
        """
        print("\n" + "="*60)
        print("采样帧图片展示")
        print("="*60)
        ...
        episode_path = self.data_path / f"chunk-{chunk_idx:03d}/episode_{episode_idx:06d}.parquet"
        ...
        print(f"Episode: {episode_idx}, Frame: {frame_idx}/{total_frames}")
        
        # 找出所有图片列
        image_columns = []
        for col in df.columns:
            # 检查是否是图片列（通常包含 rgb, image, camera 等关键字）
            if any(keyword in col.lower() for keyword in ['rgb', 'image', 'camera']):
                # 验证是否真的是图片数据
                sample_data = df[col].iloc[0]
                if isinstance(sample_data, dict) and 'bytes' in sample_data:
        ...
        print(f"找到 {len(image_columns)} 个图片列: {image_columns}")
        ...
        for col in image_columns:
            ...
            if img_data is not None and isinstance(img_data, dict) and 'bytes' in img_data:
                try:
                    ...
                    images[col] = np.array(img)
                except Exception as e:
                    print(f"  警告: 读取 {col} 失败: {e}")
        ...
        for idx, (col_name, img) in enumerate(images.items()):
            ax = axes[idx] if n_images > 1 else axes[0]
            ax.imshow(img)
            # 简化列名显示
            short_name = col_name.replace('observation.images.', '').replace('observation.', '')
            ax.set_title(f"{short_name}\n{img.shape}", fontsize=10)
            ax.axis('off')
        
        # 隐藏多余的子图
        for idx in range(len(images), len(axes) if isinstance(axes, np.ndarray) else 1):
            if isinstance(axes, np.ndarray):
                axes[idx].axis('off')
        
        plt.suptitle(f"Episode {episode_idx}, Frame {frame_idx}/{total_frames}", 
...
<<<<<<< HEAD
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='指定要查看的episode索引 (配合 --show-images 使用)'
    )
...
<<<<<<< HEAD
    if args.show_images:
        ...
        output_path = None
        if args.output:
            output_path = Path(args.output) / "sample_frame_images.png"
            Path(args.output).mkdir(exist_ok=True, parents=True)
        analyzer.visualize_sample_frame_images(
