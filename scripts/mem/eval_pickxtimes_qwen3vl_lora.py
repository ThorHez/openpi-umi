#!/usr/bin/env python3
"""Greedy held-out progress-memory evaluation for PickXtimes Qwen3-VL."""
...
from openpi.tasks.robomme.pickxtimes.qwen3vl_sft_contract import SYSTEM_PROMPT  # noqa: E402
from openpi.tasks.robomme.pickxtimes.qwen3vl_sft_contract import prompt_for_task  # noqa: E402
from openpi.tasks.robomme.pickxtimes.qwen3vl_sft_contract import validate_compact_response  # noqa: E402

DEFAULT_MODEL = Path("/data2/hzl_workspace_for_pi_mem/Qwen3-VL-4B-Instruct")
DEFAULT_MANIFEST = _ROOT / "artifacts/pickxtimes_qwen3vl_sft_seed260827/val.jsonl"
                    ...
                    "content": [
                        {"type": "video", "video": frames},
                        {
                            ...
                            "text": prompt_for_task(
                                str(row["target_color"]), int(row["required_count"])
                            ),
                        },
                    ],
                },
...
def _summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for target in range(1, 6):
        subset = [record for record in progress if record["required_count"] == target]
    ...
    sequence_correct, final_place_correct, press_correct = [], [], []
    for episode in episodes:
        ...
        places = [record for record in subset if record["expected"].get("ready_to_press") is True]
        presses = [record for record in subset if record["expected"].get("done") is True]
        final_place_correct.append(bool(places) and bool(places[-1]["correct"]))
        press_correct.append(bool(presses) and bool(presses[-1]["correct"]))
    return {
        ...
        "by_type": by_type,
        "progress_by_required_count": by_target,
        "full_progress_sequence_accuracy": sum(sequence_correct) / max(len(sequence_correct), 1),
        "final_place_ready_accuracy": sum(final_place_correct) / max(len(final_place_correct), 1),
        "press_done_accuracy": sum(press_correct) / max(len(press_correct), 1),
    }
        ...
        for start in range(0, len(rows), args.batch_size):
            ...
            for row, text in zip(batch_rows, texts, strict=True):
                ...
                expected = validate_compact_response(
                    str(row["target"]), required_count=required_count
                )
                try:
                    prediction = validate_compact_response(text, required_count=required_count)
                ...
                records.append(
                    {
                        ...
                        and prediction.get("completed_count") == expected.get("completed_count"),
                        "error": error,
                    }
                )
